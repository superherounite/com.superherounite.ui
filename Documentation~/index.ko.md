...
일반 배포에서는 `Packages/com.superherounite.ui`의 내용을 `package.json`이 root에 있는 저장소로 추출한다. `.meta` 파일을 보존하고 변경되지 않는 Semantic Version tag를 발행한다.
...
Dependency URL에 credential을 넣지 않는다. 비공개 저장소는 host의 Git credential manager 또는 SSH agent를 사용한다. 독립 저장소나 release tag를 공개하기 전에 회사가 승인한 license를 추가한다.
...
외부 release 전에는 회사가 승인한 license, 독립 저장소와 tag, 해당 Git URL로 설치한 결과, 프로젝트별 asset 및 build 설정을 사용하는 소비 프로젝트 Player Build 검증이 필요하다.
